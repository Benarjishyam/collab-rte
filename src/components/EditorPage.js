...
const EditorPage = () => {
  ...
  useEffect(() => {
    ...
    const socketInstance = io('https://socket-io-em7t.onrender.com');
    ...
    socketInstance.on('initialize-document', (documentState) => {
      ...
      if(typeof documentState === 'string' && documentState !== null){
        // Directly set the string content.
        //quillRef.current.getEditor().setText(documentState); // Use setText for strings.
        try {
          // Parse the string as JSON to get the Delta object
          const parsedDelta = JSON.parse(documentState);
          quillRef.current.getEditor().setContents(parsedDelta, 'silent');
        } catch (e) {
          console.error('Error parsing document state as JSON:', documentState, e);
...
// --------------------------------3rd effect ----------------------------------------------
  useEffect(() => {
    ...
    const handleTextChange = (delta, oldDelta, source) => {
      ...
      quillRef.current.getEditor().update();
  ...
  return (
    ...
    <div className="mainWrap">
      <div className="aside">
          <div className="asideInner">
              <div className="logo">
                  <img
                      className="logoImage"
                      src={`${process.env.PUBLIC_URL}/images/code-sync_1.png`}
                      alt="logo"
                  />
              </div>
              <h3>Connected</h3>
      ...
      <div className="editorWrap">
          <div className="editor-container"> {/* Ensure the editor container is visible and has a height */}
              <ReactQuill 
                  ref={quillRef}
                  theme="snow"
                  modules={modules}
